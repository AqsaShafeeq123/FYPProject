...
const SwappingUsers = ({ navigation, route }) => {
    ...
    const fetchSwappingUser = async () => {
        try {
            ...
            const response = await axios.get(
                `http://192.168.1.103:8000/api/get-swapping-teacher-data?day=${value.day}&startTime=${value.startTime}&endTime=${value.endTime}&timeTableId=${value.timeTableId}`
            );
    ...
    const addSwap = async () => {
        try {
            ...
            const response = await axios.post('http://192.168.1.103:8000/api/add-swapping', obj);
            console.log(response.data);
