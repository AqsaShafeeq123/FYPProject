...
const Preschedule = ({ navigation, route }) => {
    ...
    async function getSched() {
        try {
            ...
            console.log("response in getSHCED", json)

            const lst = [];
            json.forEach((element) => {
                if (!lst.includes(element.discipline)) {
                    lst.push(element.discipline);
                }
            });
            setDiscipline(lst);
            console.log(lst + 'zcsdvgs')
        } catch (error) {
    ...
    // btn 
    const handlePress = () => {
        navigation.navigate('FreeSlotPre',
            {
                Name: VALUE,
                Sdate: startDate,
                EDate: endDate,
                        ...
                        <View style={styles.pickerBox}>
                            <Picker
                                ...
                                {discipline.map((item, index) => {
                                    return (
                                        <Picker.Item label={item} color='black' style={{ backgroundColor: "#fff" }} value={item} key={index} />
                                    );
                                })}
